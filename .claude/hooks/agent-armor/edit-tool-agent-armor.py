...
# ///
"""
Agent Armor Edit/Read Tool Hook
================================

Blocks edits and reads to protected files via PreToolUse hook on Edit/Read tools.
...
def main() -> None:
    ...
    # Only check Edit and Read tools
    if tool_name not in ('Edit', 'Read'):
        sys.exit(0)
    ...
    if tool_name == 'Read':
        # For Read, only check zero-access paths
        matched, pattern = file_controller.matcher.matches(
            file_path, file_controller.zero_access_paths, case_sensitive=False
        )
        if matched:
            print(f"SECURITY: BLOCKED: No access allowed to '{file_path}' (matches: {pattern})", file=sys.stderr)
            sys.exit(2)
    else:  # Edit or Write
        # For Edit/Write, check both zero-access and read-only
